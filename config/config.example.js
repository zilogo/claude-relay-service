...
const config = {
  ...
  proxy: {
    ...
    maxSockets: (() => {
      ...
      const parsed = parseInt(process.env.PROXY_MAX_SOCKETS, 10)
    ...
    maxFreeSockets: (() => {
      ...
      const parsed = parseInt(process.env.PROXY_MAX_FREE_SOCKETS, 10)
      return Number.isFinite(parsed) && parsed >= 0 ? parsed : undefined
    })(),
  ...
  userMessageQueue: {
    ...
    timeoutMs: parseInt(process.env.USER_MESSAGE_QUEUE_TIMEOUT_MS) || 5000, // 队列等待超时（毫秒）
    lockTtlMs: parseInt(process.env.USER_MESSAGE_QUEUE_LOCK_TTL_MS) || 5000 // 锁TTL（毫秒）
  }
}
