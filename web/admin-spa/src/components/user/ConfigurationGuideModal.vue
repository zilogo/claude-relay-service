      ...
      <div class="relative flex min-h-screen items-center justify-center p-4">
        ...
        >
          ...
          <div class="max-h-[calc(100vh-200px)] overflow-y-auto px-6 py-6">
            ...
            <div
              v-if="apiKey"
              class="mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-700 dark:bg-blue-950/30"
            >
              ...
              <div class="flex items-center justify-between">
                ...
                <div v-if="apiKey.key" class="flex items-center gap-2">
                  <button
                    class="rounded-lg bg-blue-100 px-3 py-1 text-xs font-medium text-blue-700 transition-colors hover:bg-blue-200 dark:bg-blue-900 dark:text-blue-300 dark:hover:bg-blue-800"
                    @click="showFullKey = !showFullKey"
                  >
                    <i class="mr-1" :class="showFullKey ? 'fas fa-eye-slash' : 'fas fa-eye'" />
                    {{ showFullKey ? '隐藏' : '显示' }}
                  </button>
                  <button
                    v-if="showFullKey"
                    class="rounded-lg bg-green-100 px-3 py-1 text-xs font-medium text-green-700 transition-colors hover:bg-green-200 dark:bg-green-900 dark:text-green-300 dark:hover:bg-green-800"
                    @click="copyApiKey"
                  >
              ...
              <div v-if="showFullKey && apiKey.key" class="mt-3">
                <code
                  class="block break-all rounded bg-blue-100 p-2 font-mono text-xs text-blue-800 dark:bg-blue-900 dark:text-blue-200"
                >
                  {{ apiKey.key }}
                </code>
              </div>
            ...
            <!-- 配置指南组件 -->
            <EnvironmentSetupGuide :api-key="apiKey?.key || ''" :base-url="baseUrl" />
          </div>
          ...
          >
            <button
              class="rounded-lg bg-gray-100 px-6 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600"
              @click="handleClose"
            >
              关闭
            </button>
