...
class ReferralService {
  ...
  normalizeInviteRecord(record) {
    ...
    return {
      ...
      status: record.rewardIssued ? 'rewarded' : record.qualifiedAt ? 'qualified' : 'pending'
    }
  }
  ...
  async processInviteeRecharge({
    ...
    // 只累计在线支付充值金额
    record.onlineRechargeUsd =
      parseFloat(record.onlineRechargeUsd) + parseFloat(rechargeAmountUsd || 0)
